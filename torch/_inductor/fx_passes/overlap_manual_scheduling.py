...
class ManualOverlapPreservingBucketer(OverlapPreservingBucketer):
    ...
    def _check_recursive_dep(self, node: fx.Node, target_op: str, dep_dict: dict[torch.fx.Node, "OrderedSet[torch.fx.Node]"]) -> bool:
        deps: OrderedSet[fx.Node] = dep_dict[node]
        dep_counts = 0
        for dep in deps:
            if dep.op == target_op:
                dep_counts = dep_counts +1
        if dep_counts == 1:
            return False
        return True
    ...
    def bucket_collectives(self, nodes: list[fx.Node]) -> None:
        ...
        for node in collectives:
            ...
            if is_all_gather(node) and self._check_recursive_dep(node, "placeholder", self.node_ancestors):
                continue
            if is_reduce_scatter(node) and self._check_recursive_dep(self.collective_info[node].wait_node, "output", self.node_users):
                continue
            if key is not None:
